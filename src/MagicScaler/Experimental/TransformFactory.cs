...
public static class TransformFactory
{
	...
	/// <returns>An <see cref="IPixelSource"/> that provides the resulting pixel data.</returns>
	public static IPixelSource CreateConversionTransform(IPixelSource source, Guid destFormat, IColorProfile? sourceProfile, IColorProfile? destProfile)
	{
		ColorProfile? sourceProf = sourceProfile?.Get();
		ColorProfile? destProf = destProfile?.Get();
		return new ConversionTransform(source.AsPixelSource(), PixelFormat.FromGuid(destFormat), sourceProf, destProf);
	}
}
